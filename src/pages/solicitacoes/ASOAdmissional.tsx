...
export const ASOAdmissional: React.FC = () => {
  useNavigationLog();
  ...
  const carregarDados = async () => {
    try {
      ...
      const [solicitacoesData, clinicasData] = await Promise.all([
        solicitacoesService.getAll({ tipo_exame: 'ASO_ADMISSIONAL' }),
  ...
  return (
    <Box>
      <PageHeader
        ...
        subtitle="Gestão de exames admissionais para novos colaboradores"
        breadcrumbs={[
                ...
                {solicitacoes.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={8} align="center" sx={{ py: 4 }}>
                      <Typography color="text.secondary">
                        Nenhuma solicitação de ASO Admissional encontrada
                      </Typography>
                    </TableCell>
                  </TableRow>
                ) : (
                  solicitacoes.map((solicitacao) => (
                    <TableRow key={solicitacao.id} hover>
                      ...
                      <TableCell>{solicitacao.clinica_nome || '-'}</TableCell>
                      <TableCell>
      ...
      <Dialog open={dialogNovaAberto} onClose={() => setDialogNovaAberto(false)} maxWidth="md" fullWidth>
        <DialogTitle>Nova Solicitação de ASO Admissional</DialogTitle>
        <DialogContent>
