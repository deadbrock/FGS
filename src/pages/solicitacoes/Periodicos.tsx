...
export const Periodicos: React.FC = () => {
  useNavigationLog();
  ...
  const carregarDados = async () => {
    try {
      ...
      const [solicitacoesData, clinicasData] = await Promise.all([
        solicitacoesService.getAll({ tipo_exame: 'PERIODICO' }),
  ...
  return (
    <Box>
      <PageHeader
        ...
        subtitle="Gestão de exames periódicos dos colaboradores"
        breadcrumbs={[
                  ...
                  <TableRow>
                    <TableCell colSpan={8} align="center" sx={{ py: 4 }}>
                      <Typography color="text.secondary">
                        Nenhuma solicitação de exame periódico encontrada
                      </Typography>
                    </TableCell>
                  </TableRow>
                ) : (
      ...
      {/* Dialogs (mesma estrutura do ASO Admissional) */}
      <Dialog open={dialogNovaAberto} onClose={() => setDialogNovaAberto(false)} maxWidth="md" fullWidth>
        <DialogTitle>Nova Solicitação de Exame Periódico</DialogTitle>
        <DialogContent>
