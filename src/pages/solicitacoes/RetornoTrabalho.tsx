...
export const RetornoTrabalho: React.FC = () => {
  useNavigationLog();
  ...
  const carregarDados = async () => {
    try {
      ...
      const [solicitacoesData, clinicasData] = await Promise.all([
        solicitacoesService.getAll({ tipo_exame: 'RETORNO_TRABALHO' }),
  ...
  return (
    <Box>
      <PageHeader
        ...
        subtitle="Gestão de exames de retorno ao trabalho após afastamento"
        breadcrumbs={[
            ...
            <Table>
              <TableHead>
                <TableRow>
                  ...
                  <TableCell>Motivo Afastamento</TableCell>
                  <TableCell>Data Afastamento</TableCell>
                ...
                {solicitacoes.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={8} align="center" sx={{ py: 4 }}>
                      <Typography color="text.secondary">
                        Nenhuma solicitação de retorno ao trabalho encontrada
                      </Typography>
                    </TableCell>
                  </TableRow>
                ) : (
                  solicitacoes.map((solicitacao) => (
                    <TableRow key={solicitacao.id} hover>
                      ...
                      <TableCell>{solicitacao.motivo_afastamento || '-'}</TableCell>
                      <TableCell>
                        {solicitacao.data_afastamento ? formatarData(solicitacao.data_afastamento) : '-'}
                      </TableCell>
                      <TableCell>
      ...
      <Dialog open={dialogNovaAberto} onClose={() => setDialogNovaAberto(false)} maxWidth="md" fullWidth>
        <DialogTitle>Nova Solicitação de Retorno ao Trabalho</DialogTitle>
        <DialogContent>
          <Grid container spacing={2} sx={{ mt: 1 }}>
            ...
            <Grid item xs={12} md={6}>
              <TextField
                ...
                value={formNova.motivo_afastamento}
                onChange={(e) => setFormNova({ ...formNova, motivo_afastamento: e.target.value })}
                placeholder="Ex: Doença, Acidente, Licença médica"
              />
            </Grid>
            <Grid item xs={12} md={6}>
              <TextField
                ...
                value={formNova.data_afastamento}
                onChange={(e) => setFormNova({ ...formNova, data_afastamento: e.target.value })}
